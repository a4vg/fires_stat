...
g_2010 <- read.csv(file="data/GenIn2010_gourmet.csv", T)
g_2012 <- read.csv(file="data/GenIn2012_gourmet.csv", T)
g_2014 <- read.csv(file="data/GenIn2014_gourmet.csv", T)
...
```
<div>
En general podemos ver que la mayoría de los estados tienen como la causa más común 'Unknown' o desconocido. Sin embargo, en el estado de Massachusetts es común tener incendios por cocinar junto a New Jersey y en Vermont, ocurren muchas fallas por calentadores.
</div>
...
```
<div>
En los años 2010 y 2012 la diferencia no es muy amplia. Sin embargo, esta sí es visible especialmente en Wyoming y California. En el año 2014 sí podemos notar un cambio más grande. Por eso, podemos decir que la población sí está relacionada con el índice de incendios por humo y que en los estados con menos ciudadanos ocurren más incendios de este tipo.
</div>
...
```{r}
arrival2010 <- as.numeric(c(difftime(g_2010$arrival, g_2010$alarm, units='mins')))
mu2010  <- mean(arrival2010, na.rm = TRUE)
arrival2012 <- as.numeric(difftime(g_2012$arrival, g_2012$alarm, units='mins'))
mu2012 <- mean(arrival2012, na.rm = TRUE)
arrival2014 <- as.numeric(difftime(g_2014$arrival, g_2014$alarm, units='mins'))
mu2014 <- mean(arrival2014, na.rm = TRUE)
arrivalt <- c(arrival2010, arrival2012, arrival2014)
mut <- (mu2010+mu2012+mu2014)/3
alfa <-  1-0.95
...
<div>
*Conclusión*: Se rechaza la hipotesis nula, ya que p-value vale 1.011e-05 y nuestro alfa es 0.05. Podemos ver que p-value es menor y la verdadera media es 17.27185. Lo que significa que el tiempo de respuesta es un poco más de 17 minutos.
</div>
