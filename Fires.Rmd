...
reqpackages <- c("prettydoc", "plyr", "dplyr")
...
* Tipo de muestreo: No hay, es un parámetro estadístico ya que toda la data es sacada del National Fire Incident Reporting System (NFIRS) de los Estados Unidos. Si bien la participación no es obligatoria para algunos estados, los set de data referenciados en este proyecto sí incluyen todos los 50 estados con más de dos tercios de participación de los departamentos de bomberos a nivel nacional (pg.4). La Administración de Incendios de Estados Unidos (USFA) considera que el NFIRS contiene la cantidad suficiente de información que se necesita para hacer estimados razonables de las varias facetas de un problema de fuego (pg.4) [https://www.usfa.fema.gov/downloads/pdf/publications/nfirs_data_quality_report.pdf].
...
# Upload csv
causes_2010 <- read.csv(file="data/Causes2010_cleaned.csv")
causes_2012 <- read.csv(file="data/Causes2012_cleaned.csv")
causes_2014 <- read.csv(file="data/Causes2014_cleaned.csv")
...
barplot(prop.table(table(causes_2010$cause_code)), col=c("gold", "darkorchid"), ylim=c(0,0.5), main = "Porcentaje de Causas en el 2010", ylab="Frecuencia Relativa")

barplot(prop.table(table(causes_2012$cause_code)), col=c("deeppink", "chartreuse2"), ylim=c(0,0.5), main = "Porcentaje de Causas en el 2012", ylab="Frecuencia Relativa")

barplot(prop.table(table(causes_2014$cause_code)), col=c("orange", "deepskyblue"), ylim=c(0,0.5), main = "Porcentaje de Causas en el 2014", ylab="Frecuencia Relativa")
...
barplot(vh,names.arg=c("CA", "TX", "NY", "VT", "WA", "WY"), main = "2010")
...
barplot(vh,names.arg=c("CA", "TX", "NY", "VT", "WA", "WY"), main = "2012")
...
barplot(vh,names.arg=c("CA", "TX", "NY", "VT", "WA", "WY"), main = "2014")
...
causes_2010_fwAtHolid <- subset(c_2010_allFw, grepl(paste(holidays,collapse="|"), inc_date))

c_2012_allFw <- subset(causes_2012, pcc == 7)
causes_2012_fwAtHolid <- subset(c_2012_allFw, grepl(paste(holidays,collapse="|"), inc_date))

c_2014_allFw <- subset(causes_2014, pcc == 7)
causes_2014_fwAtHolid <- subset(c_2014_allFw, grepl(paste(holidays,collapse="|"), inc_date))

num_fw <- sum(nrow(c_2010_allFw), nrow(c_2012_allFw), nrow(c_2014_allFw))
num_fw_hol <- sum(nrow(causes_2010_fwAtHolid), nrow(causes_2012_fwAtHolid), nrow(causes_2014_fwAtHolid))

perc <- num_fw_hol / num_fw * 100

print("Porcentaje de incidentes con fuegos artificiales que ocurrieron en feriados vs todos los incidentes con fuegos artificiales: ", quote = FALSE)
print(perc)
```
**Conclusión**: Que sea un día festivo no está relacionado con los incendios por fuegos artificiales, pues solo un tercio de estos ocurren en días festivos.
